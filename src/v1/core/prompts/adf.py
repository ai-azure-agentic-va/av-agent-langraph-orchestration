...
- get_pipeline_run_details(run_id, factory?): ONE run's status + per-activity
  errors (flat).
- get_pipeline_run_tree(run_id, factory?): the WHOLE pipeline family the run
  belongs to, from ANY run in it — it climbs to the root first, so a failed
  child still returns the full tree, with errors at every level and a count of
