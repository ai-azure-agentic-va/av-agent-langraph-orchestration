...
# The MuleSoft gateway intermittently answers an otherwise-valid incident query
# with a 500 that succeeds verbatim on the next attempt. 500 is deliberately NOT
# in the global retryable set (a server error is usually a real bug), so this
# widens the policy for the MuleSoft GET alone — an idempotent read where a
# replay costs nothing. Do NOT reuse it on a write path.
_MULESOFT_RETRYABLE_STATUS_CODES = frozenset({500})

# Gateway trace identifiers. Whichever of these the failing response carries is
# what the MuleSoft team needs to find the request on their side — without one,
# an intermittent 500 is unattributable and can never be root-caused.
_MULESOFT_CORRELATION_HEADERS = (
    "x-correlation-id",
    "x-request-id",
    "x-mulesoft-request-id",
    "x-anypoint-request-id",
)

# Cap on how much of an error body reaches the logs. Bounded because the body is
# attacker-influencable in principle and unbounded in practice (an HTML error
# page from an intermediate proxy can be megabytes).
_ERROR_BODY_PREVIEW_LIMIT = 2048

# Best-effort scrub of credential-shaped values before an error body is logged.
# This is defence in depth, not a guarantee: it is a denylist of key NAMES, so a
# gateway echoing a secret under an unexpected label would still slip through.
# 'secret' and 'token' are matched bare so they also cover clientsecret /
# client_secret / access_token / refresh_token without enumerating each.
# The trailing ``\"?`` in the kept group is load-bearing: the gateway answers in
# JSON, so the value arrives QUOTED. Without it the value class (which excludes
# ``"``) cannot match past the opening quote and the whole substitution silently
# no-ops on exactly the bodies it exists to scrub.
_SENSITIVE_ERROR_VALUE_RE = re.compile(
    r"((?:authorization|password|secret|token|api[_-]?key|client[_-]?id)\"?\s*[:=]\s*\"?)[^,\s\"}]+",
    re.IGNORECASE,
)


def _safe_error_body_preview(response: httpx.Response) -> str:
    """Return a bounded error-body preview with credential-shaped values redacted."""

    preview = response.text[:_ERROR_BODY_PREVIEW_LIMIT]
    return _SENSITIVE_ERROR_VALUE_RE.sub(r"\1[REDACTED]", preview)
...
#   * ``assigned_to`` / ``resolved_by``                 — user CODE ('E1042')
...
_REFERENCE_FILTER_FIELDS = {
    ...
    # Caller (the person who REPORTED the ticket) — added 2026-08-11. Same two forms
    # as the pair above: user CODE or name SUBSTRING. The bundled mock fixture carries
    # no ``caller_id`` field, so a caller filter matches nothing in mock / degraded
    # fallback — deliberately zero rather than the unfiltered set an unmapped key
    # would return.
    "caller_id": "caller_id",
    "caller_id_name_contains": "caller_id",
}
...
def _normalize_transport(raw: str | None) -> str:
    """Normalize transport with strict accepted values.

    Allowed values:
    - ``mulesoft`` (default when unset/blank)
    - ``snow`` (native ServiceNow endpoint path)
    """

    value = (raw or "").strip().lower()
    if value in {"", "mulesoft"}:
        return "mulesoft"
    if value == "snow":
        return "snow"
    raise ServiceNowConfigurationError(
        "SERVICENOW_TRANSPORT must be 'mulesoft' or 'snow'"
    )
...
class ServiceNowConfig:
    ...
    # --- MuleSoft transport (default) -------------------------------------
    # Real-mode calls route through MuleSoft by default.
    # Set SERVICENOW_TRANSPORT=snow to use native ServiceNow OAuth endpoints.
    transport: str = "mulesoft"  # mulesoft | snow
    mulesoft_api_url: str | None = None
    mulesoft_client_id: str | None = None
    ...
    def from_env(cls) -> ServiceNowConfig:
        ...
        return cls._from_env_with_secrets(
            ...
            mulesoft_client_id=resolve_env_secret("MULESOFT_CLIENT_ID"),
            mulesoft_client_secret=resolve_env_secret("MULESOFT_CLIENT_SECRET"),
        )
    ...
    async def afrom_env(cls) -> ServiceNowConfig:
        ...
        (
            ...
            aresolve_env_secret("SERVICENOW_CLIENT_ID"),
            aresolve_env_secret("SERVICENOW_CLIENT_SECRET"),
            aresolve_env_secret("MULESOFT_CLIENT_ID"),
            aresolve_env_secret("MULESOFT_CLIENT_SECRET"),
        )
        return cls._from_env_with_secrets(
            client_id=client_id,
    ...
    def _from_env_with_secrets(
        ...
        *,
        ...
        # Only the direct (snow) transport builds its URL from this prefix; the
        # MuleSoft gateway ignores it and uses MULESOFT_API_URL, so the guard would
        # be a false positive there.
        if (
            _prod_env()
            and config.is_real
            and config.transport == "snow"
    ...
    @property
    def is_mulesoft(self) -> bool:
        """True when real-mode calls should go via the MuleSoft gateway."""
        return self.transport == "mulesoft"

    @property
    def is_snow(self) -> bool:
        """True when real-mode calls should hit native ServiceNow endpoints."""
        return self.transport == "snow"

    @property
    def has_mulesoft_credentials(self) -> bool:
        """True when MuleSoft gateway credentials (x-client-clientid/secret) are present."""
        return bool(self.mulesoft_client_id) and bool(self.mulesoft_client_secret)
...
SUPPORTED_FILTERS: dict[str, _FilterSpec] = {
    ...
    # last name alone works ('Alvarez', 'Chen'). PREFERRED over the *_name
    ...
    # Caller = who REPORTED the ticket (ServiceNow team, 2026-08-11). ``caller_id``
    # takes comma-separated user codes; ``caller_id_name_contains`` a name substring.
    # The wrapper also exposes ``caller_id_name`` (comma-separated EXACT 'Name (CODE)'
    # strings) — not wired: *_contains subsumes it for one person, and ``caller_id``
    # already covers the several-people case.
    "caller_id": _FilterSpec("caller_id", "passthrough"),
    "caller_id_name_contains": _FilterSpec("caller_id_name_contains", "contains"),
...
class ServiceNowClient:
    ...
    def _list_incidents_mock(
        ...
        *,
        ...
        return self._build_envelope(
            ...
            degraded=degraded,
            # Mock filters the WHOLE fixture set, so the match count IS the true
            # total. Real mode has no such luxury and must send the API's own.
    ...
    async def _list_incidents_real(
        ...
        *,
        ...
        try:
            if self.config.is_mulesoft:
                # The gateway performs OAuth itself; we mint no token and
                # authenticate via headers instead.
                payload = await self._fetch_incidents_mulesoft(
                    filters=filters, limit=limit, offset=offset
                )
            else:
                # Direct ServiceNow: when client credentials are configured, always
                # mint our own OAuth token; only fall back to a caller-supplied
                # token otherwise.
    ...
    async def _list_incidents_mulesoft(
        self,
        *,
        filters: Mapping[str, Any] | None,
        limit: int,
        ...
        """Real-mode path via MuleSoft API gateway (header-auth, no OAuth token).

        Sends the same SUPPORTED_FILTERS as query params (same encoding as the
        direct path), then authenticates via the four MuleSoft / ServiceNow
        credential headers instead of a Bearer token. The existing direct OAuth
        path (``_list_incidents_real``) is left completely unchanged.
        """
        try:
            payload = await self._fetch_incidents_mulesoft(
                filters=filters, limit=limit, offset=offset
            )
            return self._envelope_from_real_payload(payload, limit=limit, offset=offset)
        except (httpx.HTTPError, ServiceNowError) as exc:
            if not self.config.fallback_to_mock:
                raise ServiceNowError(
                    "MuleSoft ServiceNow request failed and fallback is disabled"
                ) from exc
            logger.error(
                "servicenow.mulesoft_failed_fallback_to_mock",
                extra={
                    "event": "servicenow.mulesoft_failed_fallback_to_mock",
                    "error_type": type(exc).__name__,
                    "error": str(exc),
                },
            )
    ...
    async def _fetch_incidents_mulesoft(
        self,
        *,
        filters: Mapping[str, Any] | None,
        limit: int,
        ...
        """HTTP GET against the MuleSoft /incidents endpoint.

        Authentication is entirely header-based — no OAuth token endpoint:
        - ``servicenow-client-instance``  : MuleSoft tenant/instance label
        - ``x-client-clientid``             : MuleSoft API client ID
        - ``x-client-clientsecret``         : MuleSoft API client secret
        - ``servicenow-client-id``        : forwarded ServiceNow client ID
        - ``servicenow-client-secret``    : forwarded ServiceNow client secret
        """
        api_url = self.config.mulesoft_api_url
        if not api_url:
            raise ServiceNowError(
                "MULESOFT_API_URL is not configured — set it in the environment "
                "or set SERVICENOW_TRANSPORT=snow to use native ServiceNow endpoints."
            )
        ...
        request_url = f"{api_url.rstrip('/')}?{query}"

        # All credential headers are required; warn loudly if any are blank so the
        # first 401 from MuleSoft is immediately diagnosable.
        headers = {
            "servicenow-client-instance": self.config.mulesoft_snow_instance or "example-instance",
            "x-client-clientid": self.config.mulesoft_client_id or "",
            "x-client-clientsecret": self.config.mulesoft_client_secret or "",
            "servicenow-client-id": self.config.client_id or "",
            "servicenow-client-secret": self.config.client_secret or "",
        }
        if not self.config.has_mulesoft_credentials:
            logger.warning(
                "servicenow.mulesoft_credentials_missing",
                extra={
                    "event": "servicenow.mulesoft_credentials_missing",
                    "hint": "Set MULESOFT_CLIENT_ID and MULESOFT_CLIENT_SECRET",
                },
            )

        logger.info(
            "servicenow.mulesoft_request",
            extra={
                "event": "servicenow.mulesoft_request",
                "request_url": request_url,
                "snow_instance": self.config.mulesoft_snow_instance,
                "limit": limit,
                "offset": offset,
                "filter_keys": sorted(params.keys()),
                # credential headers intentionally omitted
            },
        )
        client = self._get_http_client()

        @http_retry_async(additional_status_codes=_MULESOFT_RETRYABLE_STATUS_CODES)
        async def _do_get() -> httpx.Response:
            resp = await client.get(request_url, headers=headers)
            if resp.status_code == 202:
                raise httpx.HTTPStatusError(
                    "MuleSoft returned 202 Accepted (async result not yet available)",
                    request=resp.request,
                    response=resp,
                )
            # Log the diagnostics HERE, not in the caller's except: a 500 that a
            # retry rescues never reaches the caller, and that is precisely the
            # case being investigated. Recording only the final failure would let
            # the fix hide the very problem it is meant to expose.
            if resp.status_code >= 400:
                logger.error(
                    "servicenow.mulesoft_http_error",
                    extra={
                        "event": "servicenow.mulesoft_http_error",
                        "status_code": resp.status_code,
                        "content_type": resp.headers.get("content-type"),
                        "response_body_length": len(resp.content),
                        "response_body_preview": _safe_error_body_preview(resp),
                        "correlation_headers": {
                            name: value
                            for name in _MULESOFT_CORRELATION_HEADERS
        ...
        try:
            ...
            raise ServiceNowError(
                f"MuleSoft ServiceNow endpoint returned HTTP {exc.response.status_code}"
            ) from exc
        try:
            payload = response.json()
        except json.JSONDecodeError as exc:
            raise ServiceNowError(
                f"MuleSoft ServiceNow endpoint returned non-JSON response "
                f"(status {response.status_code}, content-type: "
                f"{response.headers.get('content-type', 'unknown')})"
            ) from exc

        # Log response shape for diagnosability (same pattern as the direct path).
        ...
        log_level = logger.warning if record_count is None else logger.info
        log_level(
            "servicenow.mulesoft_response_shape",
            extra={
                "event": "servicenow.mulesoft_response_shape",
    ...
    def _build_query(
        self, filters: Mapping[str, Any] | None, limit: int, offset: int
    ) -> str:
        """Encode the shared ``/incidents`` query string (transport-agnostic).

        The MuleSoft gateway and the direct ServiceNow path expose the SAME query
        contract (``?number=INC...&limit=...&offset=...``), so both build the query
        the same way; only the base URL and auth headers differ.
        """
    ...
    def _log_response_shape(self, payload: Any, *, request_path: str) -> None:
        ...
        # shape is the cause. status_code is always 200 here (raise_for_status upstream).
        if isinstance(payload, Mapping):
    ...
    def _mulesoft_headers(self) -> dict[str, str]:
        """Build the five MuleSoft gateway headers.

        The gateway needs BOTH its own subscription credentials (``x-client-*``) and
        the downstream ServiceNow credentials (``servicenow-*``), plus the target
        instance. Each header is only set when its value is present so a missing
        secret surfaces as a gateway auth error rather than a header carrying the
        literal string 'None'. Secrets are never logged.
        """

        headers: dict[str, str] = {}
        if self.config.mulesoft_snow_instance:
            headers[_MULESOFT_HEADER_SNOW_INSTANCE] = self.config.mulesoft_snow_instance
        if self.config.mulesoft_client_id:
            headers[_MULESOFT_HEADER_SUBSCRIPTION_ID] = self.config.mulesoft_client_id
        if self.config.mulesoft_client_secret:
            headers[_MULESOFT_HEADER_SUBSCRIPTION_SECRET] = self.config.mulesoft_client_secret
        if self.config.client_id:
            headers[_MULESOFT_HEADER_SNOW_CLIENT_ID] = self.config.client_id
        if self.config.client_secret:
            headers[_MULESOFT_HEADER_SNOW_CLIENT_SECRET] = self.config.client_secret
        return headers
    ...
    async def _fetch_incidents(
        ...
        *,
        ...
        self._log_response_shape(
            payload, request_path=self.config.incident_list_api_prefix
        )
        return payload
    ...
    def _build_envelope(
        ...
        *,
        ...
        return {
            "result_count": result_count,
            # Total matches across ALL pages, or None when the caller could not
            # establish one. NEVER fall back to result_count here: in real mode that
            # is only the PAGE count, so the fallback laundered "unknown" into a
            # number that reads as an authoritative total and silently undercounts
            # (a 28-match query reporting 10). Unknown must stay unknown.
