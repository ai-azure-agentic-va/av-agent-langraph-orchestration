...
A single call site can widen that set via
``http_retry_async(additional_status_codes=...)`` — see the MuleSoft GET in
``clients/servicenow.py``. Per-call-site, never global: the default policy
must stay safe for non-idempotent requests such as the OAuth token POST.
...
def _is_retryable(
    ...
    *,
    ...
    retryable_status_codes = _RETRYABLE_STATUS_CODES | additional_status_codes
...
def http_retry_async(
    *,
    ...
    ``additional_status_codes`` widens the retryable set for ONE call site
    only. It exists because a specific upstream returns a transient code the
    global policy treats as fatal (the MuleSoft gateway intermittently 500s on
    a request that succeeds on replay). Only ever opt in an IDEMPOTENT request:
    the default set stays narrow so a genuine server bug on a write path still
    fails fast instead of being replayed.
    """

    return retry(
        retry=retry_if_exception(
            lambda exc: _is_retryable(exc, additional_status_codes=additional_status_codes)
        ),
